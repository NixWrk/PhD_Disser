...
lv=np.asarray(nib.load(root/'agent_drafts/sequence02_phase00_LV_body_v03/LV_visible_body_agent_HU190.nii.gz').dataobj).astype(bool)
rv=np.asarray(nib.load(root/'agent_drafts/sequence02_phase00_RV_body_v02/RV_visible_body_agent_HU190.nii.gz').dataobj).astype(bool)
out=root/'body_review';out.mkdir(exist_ok=True)
for page,start in enumerate([4,12,20],1):
    ...
    for ax,k in zip(axs.ravel(),range(start,start+8)):
        ...
        ax.set_title(f'k={k} | partial LV green, RV blue')
...
for ax,p in zip(axs,planes[:2]):
    ...
    ax.set_title(p['name']+' | PARTIAL DRAFTS');ax.set_xlabel('u, mm');ax.set_ylabel('v, mm');ax.grid(alpha=.15)
