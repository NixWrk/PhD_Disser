...
p=inputParser;addParameter(p,'OutputDir','output/exploratory/lateral_fat_inverse_20260916/broad');
...
for ci=1:size(cases,1)
 ...
 limits=repmat([.1,1000],3,1); % Numerical starting box only, not physiological bounds.
 ...
 seeds=[3,5,10;8,200,60];starts=log(seeds(:,1:npar));solutions=zeros(2,npar);values=zeros(2,1);flags=zeros(2,1);iters=zeros(2,1);
 lastx=[];evals=0;maxres=0;maxrec=0;t=tic;
 history=struct([]);previous_rmse=Inf;previous_z=[];stopreason="interior_stationary_point";
 for stage=0:3
  for st=1:2
  ...
  if ~any(boundary),break;end
  if stage>=2 && abs(previous_rmse-rmse)<.001 && max(abs(lastz-previous_z))<.02
   stopreason="boundary_plateau_no_finite_estimate";break;
  end
  if stage==3,stopreason="numerical_boundary_unresolved";break;end
  previous_rmse=rmse;previous_z=lastz;lb(lower)=lb(lower)-log(10);ub(upper)=ub(upper)+log(10);
  starts=solutions;lastx=[];
 end
...
% Profile the weak lung parameter well beyond literature; each point refits rho1.
...
profile_values=10.^(-1:6);a=lit(lit.frequency_hz==50000,:);getrho=@(name)a.rho_ohm_m(a.tissue==name);
...
for fixed=profile_values
 ...
 [px,value,~,flag]=lsqnonlin(@residual,log(4),log(.001),log(1e5),opt);assert(flag>0);
