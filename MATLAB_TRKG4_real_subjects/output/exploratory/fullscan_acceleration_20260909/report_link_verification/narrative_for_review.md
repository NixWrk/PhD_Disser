...
**Частичный полный перебор, остановлен автором 09.09.2026.**
[Интерактивная карта геометрической допустимости: 3D и матрица центров](../MATLAB_TRKG4_real_subjects/output/full_pose_scan_fullscan_20260908_v2/geometry_report/geometry_map.html).
В сохранённых 408 блоках проверены 26 112 из 58 081 центров, по 41 углу в каждом
(1 070 592 положения). Во всех углах проходят 2 440 центров, во всех углах
отклоняются 6 109; для 17 563 результат зависит от поворота. Зелёный цвет
означает большую долю прошедших геометрическую проверку углов, красный —
большую долю отказов, серый — отсутствие проверки. Это результат контроля
дискретных электродных площадок на данной сетке, а не карта расхождения
импеданса с экспериментом: стадия CEM этого полного перебора не выполнена.
Отказ не устанавливает физическую невозможность монтажа. Для большинства
отказов сохранён общий код контроля без указания размера, электрода и
конкретного нарушенного условия.

**Окно карты после уточнения автора 09.09.2026:** u = −80…80 мм;
v = −120…120 мм, все 41 угла сохранены. В отображаемом окне проверены
17 432 центра и 714 712 поз; проходят 516 503 позы. Все углы проходят
в 2 437 центрах, ни один — в 460. Исходные результаты за пределами окна
сохранены. Автор связывает граничные отказы с выходом электродов на руки
или нижний срез STL. Реализация ускорения и отличие от раннего PEM-поиска
зафиксированы в [техническом разборе](../MATLAB_TRKG4_real_subjects/notebooks/full_scan_2026_09_08/ACCELERATION_REVIEW_20260909.md).
После согласования автора реализованы маска искусственных рук и плоских
срезов, кеш локальной геометрии, повторное использование точных CEM-контактов
и перекрывающихся частей Green, публикация карты после каждого блока.
Исходный прогон v2 остаётся остановленным; новые проверки имеют отдельные
контракты. В контрольной сетке из двух центров рассчитаны все 82 позы и
738 импедансов. Независимый полный EIDORS по девяти размерам лучшей позы
отличается максимум на 1,042·10⁻⁸ Ом. Совпадение подтверждает численную
реализацию; оно не является независимой физической валидацией.

[Частичная CEM-карта двух контрольных блоков на полном окне u = −80…80](../MATLAB_TRKG4_real_subjects/output/full_pose_scan_accel_pilot_20260909_01/live/report/full_scan.html)
содержит уже расчётную RMSE относительно девяти экспериментальных импедансов.
Чем меньше RMSE, тем зеленее точка. Отказы QC, попадание на исключённую
поверхность и отсутствие расчёта разделены. У исторических отказов точная
причина может оставаться неизвестной: контактные площадки таких поз ранее не
сохранялись. Контрольный запуск автоматически остановлен после двух блоков: обработаны
128 центров и 5 248 поз, из них CEM рассчитан для 3 813, исключены по поверхности
1 021, геометрический отказ сохранили 414. Проверка возобновления подтвердила
неизменность первого готового блока. Глобальный минимум всей сетки не установлен.
Маска опирается на параметры подготовки этой модели; она является явным
геометрическим допущением, а не новой анатомической сегментацией.
На проверенной последовательности 738 сборок кеш геометрии сократил время
12,822 → 10,961 с (14,5%) с точным совпадением узлов, граней и QC. Это
локальный замер; ускорение полного перебора ещё не измерено.
