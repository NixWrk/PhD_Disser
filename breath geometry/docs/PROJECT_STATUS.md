...
- Пространственная карта Δтолщины по телу: измеренный локальный шум `2.626 мм` одного
  порядка с ожидаемым эффектом, поэтому карта не отчитывается до Gate 1B.
...
2. Трек грудной стенки ведётся параллельно J1.2, а не после него. Gate 1 в текущем виде
   ...
   Шаг (a) выполнен: измеритель пригоден для одного числа по субъекту, но не для
   3D-карты, а раздельные жир/мышца ограничены HU-сегментацией. Поэтому к (b) и (c)
   добавляется независимый приоритет — заменить HU-окна валидированной сегментацией;
   без неё раздельные ткани остаются `marginal` с шумовым полом около `±1.1 мм`.
