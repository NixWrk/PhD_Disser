...
[Крупный вид фактических электродных площадок и проверка минимума C01](../MATLAB_TRKG4_real_subjects/output/exploratory/scan_localization_20260910/best_pose_review/index.html).
...
### Что подтверждает минимум C01 и чего он не доказывает

Повторный расчёт RMSE из всех 475 090 сохранённых FEM-векторов подтвердил
единственность C01 среди рассчитанных допустимых параметрических поз:
2,881827921 Ом. Следующая поза имеет тот же центр и φ = 13° вместо 14°;
её RMSE равна 2,931431723 Ом. Разность составляет 0,049603802 Ом.
В полном локальном окне 3 × 3 × 3 имеются 15 допустимых поз и 12 отказов QC,
непроверенных поз нет. C01 строго лучше всех остальных допустимых соседей.
Это доказательство перебором для **дискретного допустимого множества при
фиксированных ρ**, с точностью сохранённых численных решений. Оно не
устанавливает минимум непрерывного размещения или всей незавершённой сетки.

| Сдвиг одного параметра от C01 | RMSE, Ом |
|---|---:|
| u − 1 | Отказ геометрического QC |
| u + 1 | 10,255963 |
| v − 1 | 10,298911 |
| v + 1 | Отказ геометрического QC |
| φ − 1° | 2,931432 |
| φ + 1° | 5,844396 |

Значения в недопустимых позах не являются измеренными большими ошибками.
Они не позволяют построить центральные производные по всем координатам.
Положение на долях шага не проверено; гладкость функции при изменении
дискретных площадок также не установлена. Проверка положительности
приближённого гессиана здесь не была бы доказательством физической локализации.

**Детерминированная чувствительность к экспериментальному вектору.**
Для лучшего предсказания z₀, конкурента zⱼ и исходного экспериментального
вектора y минимальное RMS-изменение данных до равенства квадратов ошибок равно
(‖zⱼ−y‖² − ‖z₀−y‖²)/(2√9‖zⱼ−z₀‖). Равенство достигается изменением данных
в направлении zⱼ−z₀; это следует из неравенства Коши–Буняковского.
Минимум по всем другим готовым предсказаниям составляет 0,077441953 Ом
и достигается для конкурента с φ = 13°. Численно проверено, что изменение
в 1,01 раза больше меняет порядок этих двух поз. Это радиус устойчивости
выбора к заданному классу изменений данных при неизменной FEM-библиотеке,
а не оценка реального шума, погрешности прибора или доверительного интервала.

**Исключение размера при локализации.** Для каждой из девяти проверок поза
заново выбиралась по восьми оставшимся размерам среди всей готовой библиотеки.
Это отдельная проверка от исключения размера при подборе h. C01 остаётся
победителем только в трёх из девяти случаев. Без размера 50 мм лучший центр
смещается на 33,000 мм, а прогноз этого исключённого размера имеет ошибку
+40,344 Ом. Такая зависимость выбора от размерного ряда не позволяет
объявлять C01 независимо восстановленной фактической установкой.

**Происхождение фиксированных ρ.** В нынешнем переборе ρ не варьировались.
Сохранённый более ранний протокол `auditfix_20260908` содержит 100 совместных
оценок пяти параметров и 24 различных пары ρ. Он завершён по бюджету
(`budget_reached`), а не по доказанной сходимости. История охватывает
ρ₁ = 4,614666…6,114666 и ρ₂ = 21,706947…32,000000 Ом·м; это фактически
проверенные значения, а не доверительные границы. Хеш исходной функции
совпадает с записанным в контракте: её критерий использовал Huber-ошибку,
наклон размерной кривой, штрафы по глубине и предварительное предпочтение
ρ лёгкого. Текущая карта минимизирует обычную RMSE. Поэтому текущая поза
не является доказанным совместным оптимумом положения и сопротивлений.

В крупном представлении показаны фактические треугольники всех 36 CEM-контактов
C01 с переключением девяти размеров. При чтении из сохранённого геометрического
блока площади совпали с ранее экспортированными значениями до 7,11·10⁻¹⁴ мм²,
центры площадей — до 5,41·10⁻¹³ мм. Такая проверка подтверждает идентичность
визуализируемых расчётных контактов, но не их совпадение с наклейкой в эксперименте.
...
Отчёт объединяет три связанные задачи. Первая состоит в построении прямой трёхмерной модели и описании локального кандидата совместной оценки двух удельных сопротивлений и общей позы сборок. Вторая состоит в проверке геометрических величин, которыми реальная анатомия может быть представлена в аналитической модели. Третья состоит в сравнении десяти вариантов прямого расчёта при фиксированных параметрах и в формулировке программы выбора вычислительно экономного приближения.
...
## 20. Итоговые положения и границы достигнутого результата
...
2. **Материальное допущение.** В сравнении упрощений используются ровно два сопротивления. Все нелёгочные ткани, включая поддиафрагмальные, объединены. Последовательность из трёх и более слоёв не увеличивает автоматически число материальных параметров.
...
Настоящая научная редакция сохраняет результаты выполненных расчётов и расширяет их методическое описание. Новая оптимизация сопротивлений, положения электродов или семейства упрощённых моделей при подготовке текста не проводилась. Сохранённые вычислительные ячейки остаются первоисточником таблиц и рисунков; текстовые предложения будущих исследований не являются дополнительными численными результатами.

## 21. Методические источники и связь с основным корпусом

### 21.1. Проверенные внешние источники
...
### 21.2. Локальная библиотека и границы библиографической проверки
