...
TARGET = ROOT.parent / 'Colab Notebooks' / '20.10_КТ_FEM_локализация_боковых_матриц_МГТУ.ipynb'
...
def main():
    ...
    if 'corpus_integration' in original.metadata:
        notebook.metadata['corpus_integration'] = copy.deepcopy(original.metadata['corpus_integration'])
    notebook.metadata['scientific_revision'] = {
        'date': date.today().isoformat(), 'source_notebook_sha256': original_hash,
        'narrative_sha256': digest(narrative),
        'saved_scan_chapter_source_sha256': digest(ROOT / 'notebooks/TRKG4_saved_scan_scientific.md'),
        'saved_scan_chapter_manifest_sha256': digest(ROOT / 'output/exploratory/scan_localization_20260910/scientific_notebook/chapter_manifest.json'),
        'scope': 'scientific prose and saved-scan chapter with six figures; previous numerical outputs reused without execution',
    ...
    report = {
        ...
        'files_sha256': {str(p.relative_to(ROOT.parent)): digest(p) for p in [destination, html_path, narrative, ROOT / 'notebooks/TRKG4_saved_scan_scientific.md']},
    }
