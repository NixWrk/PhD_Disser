...
def build_report(scan, output):
    ...
    payload = dict(u=scan["u"].tolist(), v=scan["v"].tolist(), phi=scan["phi"].tolist(),
                   state=encode(s, "u1"), xyz=encode(c[["x_mm","y_mm","z_mm"]], "<f8"),
...
def write_png(scan, path):
    ...
    for ax, value, title in zip(axes,values,titles):
        ax.imshow(np.zeros(grid),origin="lower",extent=[-120.5,120.5,-120.5,120.5],
        ...
        im=ax.imshow(np.ma.masked_invalid(value.reshape(grid)),origin="lower",
                     extent=[-120.5,120.5,-120.5,120.5],cmap=cmap,vmin=0,vmax=100,interpolation="none",rasterized=True)
        ax.set(title=title,xlabel="u, мм",ylabel="v, мм",xticks=np.arange(-120,121,40),yticks=np.arange(-120,121,40))
...
Она не устанавливает анатомическую невозможность размещения электродов. Прямой край серой области связан с порядком перебора центров.</p>
...
document.getElementById('subtitle').textContent='Сохранены '+fmt(Q.committed_blocks)+' блоков: '+fmt(Q.centres_checked)+' центров × 41 угол. Вся заданная сетка: 241 × 241 центр × 41 угол.';
...
function hover(c,value){
 ...
 let t='Центр '+(c+1)+'<br>u='+u+'; v='+v+' мм<br>';
...
let selected=Q.default_centre_index-1;
function point(c){
 selected=c;let text='<b>Центр '+(c+1)+'</b> · u='+D.u[c%D.u.length]+' мм, v='+D.v[Math.floor(c/D.u.length)]+' мм<br>';
...
async function start(){
 ...
 {margin:{l:62,r:80,t:8,b:60},xaxis:{title:{text:'u, мм'},dtick:40,range:[-120.5,120.5]},yaxis:{title:{text:'v, мм'},dtick:40,range:[-120.5,120.5],scaleanchor:'x'},uirevision:'grid'},config),
 point(selected),
...
def main():
    ...
    args=p.parse_args()
    scan=load_geometry(args.input_dir)
