...
Минимальный полезный результат — диапазон изменения профиля тканей от правой боковой кожи
до лёгкого в реальной зоне электродов, с раздельными длинами жира, мышцы, кости и прочих
тканей там, где качество сегментации это позволяет.
...
1. Batch-подготовка всех дыхательных наборов с protocol labels и provenance.
2. Надёжная сегментация лёгких, тела, рёбер и тканей в исходном FOV.
3. Deformable registration с независимым landmark TRE и контролем folding/sliding.
4. Привязка измерений к реальным электродам и к карте FEM-чувствительности.
5. Базовые модели деформации и ансамбль по неизвестной амплитуде выдоха.
6. Сквозная оценка ошибки не только в миллиметрах, но и в `ΔZ`.
