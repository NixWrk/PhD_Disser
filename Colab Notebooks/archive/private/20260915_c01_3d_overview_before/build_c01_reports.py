...
def build(which=None):
    files=['geometry.json','geometry_volumes.csv','geometry_mask_qc.csv','fem_acceptance.json',
      ...
      'analytic_quadrature_qc.csv','pem_library.csv','reference_holdouts.csv']
...
"""
    for key in [which] if which else NAMES:
        nb=nbf.v4.new_notebook(cells=cells_for(key,setup),metadata={
          ...
          'source_directory':OUT.relative_to(PIPE).as_posix(),'input_sha256':expected,'generator_sha256':sha(Path(__file__)), 'reader_source_sha256':sha(Path(__file__).with_name('c01_report_text.py')), 'table_source_sha256':sha(PIPE/'tools/c01_report_tables.py')}})
        ...
        roles={'20.10':'Геометрия C01, происхождение выбора и текущий прямой контроль.', '20.13':'Геометрические правила и новые маски C01.','20.14':'Две проверки: известные синтетические сопротивления и реальные измерения.',
               '20.16':'Основное сравнение19 моделей на65 состояниях C01; методика, результаты, библиотека и ограничения переноса.',
