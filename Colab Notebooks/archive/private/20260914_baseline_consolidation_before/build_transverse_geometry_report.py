...
def build():
    ...
    **Плоский слой «фон–лёгкое–фон» с одинаковой нижней границей под всей сборкой исключён из основных кандидатов.** Он навязывает анатомии полный нижний слой фоновой ткани. Здесь фон появляется только там, где его действительно задаёт соответствующая лёгочная область. Число пересечений вдоль отдельных лучей может различаться; это не вводит третье сопротивление.
    ...
    где $Z$ — передаточный импеданс (в действительной квазистатической модели — передаточное сопротивление), $\rho_1$ — удельное сопротивление фоновой ткани; $AM,BM,AN,BN$ — расстояния между указанными электродами; аргументы $h,q$ у ядер для краткости опущены.

    Номинальные координаты равны $(-L/2,-L/4,L/4,L/2)$, где $L$ — номинальная длина сборки; в этой аналитической формуле $L$ выражена в метрах. При $q=1$ ряд исчезает и получается однородный предел $Z=8\rho_1/(3\pi L)$. В вычислении оставлены 256 членов; диапазон контрастов обеспечивает быстрое убывание $|k|^n$. В первой модели во всех четырёх ядрах используется $h=h(0)$.
    ...
    **Результат геометрической проверки.** Среднеквадратичное расхождение уменьшилось с **13,06 до 1,57 мм**, а доля несовпадающих знаков — с **38,8 до 6,2%** при переходе от общей кривизны к переменной форме. Контроль одной интерполяции исходного поля даёт около 0,10 мм и 0,29%. Следовательно, основное различие между двумя приближениями превышает ошибку самого геометрического интерполирования.
    ...
    где $u$ — электрический потенциал, $\sigma$ — проводимость, $\mathbf x$ — пространственная координата, $\Omega_{\mathrm{лёгкого}}$ — лёгочная область, $\rho_1,\rho_2$ — удельные сопротивления двух тканей.
    ...
    **Основное сравнение состояний выполнено с точечной электродной моделью (PEM).** Для каждого из 36 контактов выбран ближайший поверхностный узел к центроиду фактической площадки. Точечные контакты внутри пространственной группы общие; токовая и сопряжённая измерительная задачи решаются на одной сетке. Расстояние от центроида площадки до узла составляет 0,42–3,66 мм. Поэтому точечное приближение включает также дискретизацию положения контакта.

    **Отдельный опыт выполнен с полной электродной моделью (CEM).** На каждой площадке потенциал электрода постоянен, потенциал ткани связан с ним контактным сопротивлением, а интегральный ток задан. Для измерительных площадок интегральный ток равен нулю; локальное перераспределение тока при этом возможно. Принято удельное контактное сопротивление $z_c=1{,}59155\cdot10^{-4}$ Ом·м², перенесённое из конфигурации проекта. Номинальная площадь равна 70,686 мм², воспроизведённые дискретные площади лежат в пределах **70,783–94,195 мм²**. Площади и центроиды всех площадок совпали с сохранёнными данными выбранного кандидата; действующий геометрический контроль пройден. Это проверка воспроизведения принятого дискретного контакта, а не независимое измерение его физической площади.
    ...
    Нормировка по контрасту служит сокращению вычислений. **Физическая постановка остаётся двухпараметрической по реальным $\rho_1$ и $\rho_2$**: все основные заданные состояния и ограничения инверсии проверяются в Ом·м. Вспомогательные нормированные задачи $\rho_1=1$, $\rho_2=q$ не выдаются за физиологические состояния.
    ...
    **Рисунок 3. Подгонка импедансов при истинных $\rho_1=4$ и $\rho_2=18$ Ом·м.** Слева показаны девять заданных КТ/FEM-импедансов и прогнозы моделей после восстановления сопротивлений; в центре — остатки «модель минус референс» для всех моделей, справа — увеличенный масштаб остатков двух пространственных вариантов. Горизонтальная ось — номинальный размер сборки. Близкие линии слева не обязательно означают близкие тканевые параметры; для оценки малых расхождений служат правая панель и таблица ниже.
    ...
    Предыдущее сравнение использовало точечные контакты. Чтобы проверить, сохраняется ли результат при конечной площади, в синтетическом контрольном состоянии исходная двухтканная КТ/CEM-модель сформировала девять импедансов при $\rho_1=4$ и $\rho_2=16$ Ом·м. Затем две пространственные аппроксимации независимо восстановили оба сопротивления прямыми CEM-решениями с теми же площадками и тем же $z_c$.
    ...
    **Рисунок 6. Изменение оценки $\rho_2$ при одноканальном возмущении импеданса.** По горизонтали указан размер единственной изменённой сборки. По вертикали — максимальное по 16 состояниям и двум знакам возмущения относительное изменение оценки $\rho_2$ по сравнению с её невозмущённой оценкой. Это мера локальной устойчивости результата данного алгоритма, а не полная ошибка относительно истинного сопротивления.
    ...
    Здесь выполнена собственная проверка для исходной КТ-геометрии и точечных электродов в синтетическом контрольном состоянии $\rho_1=4$ и $\rho_2=16$ Ом·м. По прямому и сопряжённому полям рассчитана поэлементная чувствительность

    $$S_e=\frac{1}{\rho_e}\int_{\Omega_e}\nabla u_I\cdot\nabla u_V\,dV,$$

    где $e$ — индекс конечного элемента, $\Omega_e$ — объём $e$-го элемента, $u_I$ — потенциал прямой задачи при единичном токе токовой пары, $u_V$ — потенциал сопряжённой задачи для измерительной пары при единичном токе, $\rho_e$ — удельное сопротивление ткани элемента, а $S_e$ — производная $Z$ по локальному $\ln\rho_e$. Величина $S_e$ уже учитывает объём элемента. Проверено правило суммы $\sum_eS_e=Z$ для точечной резистивной модели. Отдельные элементы могут иметь отрицательную чувствительность; это свойство передаточного измерения, а не отрицательная проводимость.
    ...
    **Рисунок 7. Распределение чувствительности по расстоянию до кожи.** Слева показана накопленная сумма $|S_e|$ для сборок 50, 90 и 140 мм, нормированная на сумму по всему телу. Пунктирные вертикали соответствуют $L/3$ каждой сборки. Справа показана доля абсолютной чувствительности глубже $L/3$: отдельно среди всех тканей и отдельно внутри лёгкого. Во втором случае знаменатель содержит только лёгочные элементы, поэтому две кривые нельзя трактовать как доли одного общего целого.
    ...
    Для сборки 140 мм глубже **46,67 мм** остаётся **{fmt_pct(deep140.absolute_beyond_pct)}%** абсолютной чувствительности всего объёма и **{fmt_pct(deep140.lung_absolute_beyond_pct)}%** абсолютной чувствительности лёгкого. Значения для всех размеров приведены ниже. Даже небольшая доля от суммарного чувствительного объёма может быть существенной частью лёгочного отклика. Поэтому критерий исключения глубокой границы должен оцениваться по ошибке восстанавливаемого $\rho_2$, а не только по отношению геометрической глубины к длине сборки.
