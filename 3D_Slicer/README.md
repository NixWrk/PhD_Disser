# 3D_Slicer — историческая попытка автоматической сегментации

> **Статус на 23.08.2026:** неудачный исторический прототип. Каноническая
> сегментация КТ добровольцев на вдохе выполнялась автором вручную в Inobitec.

В папке хранится код предполагаемой цепочки DICOM → TotalSegmentator →
постобработка → QC → экспорт. В Git нет исходных изображений, итоговых labelmap,
STL или подтверждённого сравнения с ручной сегментацией.

Аудит обнаружил несовместимость текущего `--ml`-режима с постобработкой,
нефиксированный маппинг классов TotalSegmentator, формально некорректную метрику
QC и неподтверждённую геометрию COMSOL-экспорта. Поэтому код нельзя использовать
для научных выводов или реальных медицинских данных без отдельного восстановления
и валидации.
...
- считать папку историей разработки, а не источником канонических сегментаций;
- не запускать на реальных данных и не публиковать DICOM/NIfTI/STL;
- возможное восстановление проводить только отдельно, на обезличенном
  синтетическом наборе, с проверкой против ручных масок Inobitec.

## Ранее зафиксированное техническое описание

Текст ниже описывает предполагавшийся пайплайн и может содержать устаревшие
инструкции. Он не подтверждает работоспособность или научную валидность.
