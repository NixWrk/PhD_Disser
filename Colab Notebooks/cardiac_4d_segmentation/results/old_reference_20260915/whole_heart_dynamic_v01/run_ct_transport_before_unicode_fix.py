...
ref=sitk.ReadImage(str(seq/(s['reference_phase']+'.nii.gz')),sitk.sitkFloat32)
ref_masks={ch:sitk.ReadImage(p,sitk.sitkUInt8) for ch,p in s['reference_masks'].items()}
...
for idx in s['phase_order']:
 ...
 fixed=sitk.ReadImage(str(seq/(pid+'.nii.gz')),sitk.sitkFloat32);assert_same_geometry(ref,fixed,pid)
 ...
 if idx:
  ...
  sitk.WriteTransform(transform,str(out/'target_to_reference.h5'))
 ...
 for ch,m in ref_masks.items():
  ...
  path=out/(ch+'.nii.gz');sitk.WriteImage(warped,str(path),True);records[ch]={'file':path.name,'sha256':sha(path),'volume_ml':float(arr.sum()*np.prod(fixed.GetSpacing())/1000)}
