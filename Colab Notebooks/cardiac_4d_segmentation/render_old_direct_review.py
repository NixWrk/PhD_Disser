"""Render deidentified axial review pages from direct per-phase candidate masks."""
from pathlib import Path
import numpy as np,nibabel as nib
import matplotlib;matplotlib.use("Agg")
import matplotlib.pyplot as plt
import argparse
p=argparse.ArgumentParser();p.add_argument("--review",required=True,type=Path);root=p.parse_args().review
...
for idx in [0,8]:
 ...
 for page,start in enumerate(range(2,50,12),1):
  ...
  for ax,k in zip(axs.flat,range(start,start+12)):
   ...
   ax.set_xlim(90,375);ax.set_ylim(365,85);ax.set_title(f"Прямые контуры: фаза {idx*5}%, срез {k}");ax.grid(alpha=.15);ax.tick_params(labelsize=7)
  fig.suptitle("ЛЖ — зелёный; ПЖ — голубой; ЛП — оранжевый; ПП — розовый. Контуры без HU-порога; не приняты как референс.",fontsize=12)
