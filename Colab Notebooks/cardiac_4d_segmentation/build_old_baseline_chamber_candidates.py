"""Reconstruct the frozen OLD baseline candidate version from local contour specs.
Set OLD_BASELINE_ROOT to the local directory containing the contour specs.
Use a new directory for reruns; no anatomical acceptance is inferred.
"""
import os
...
root=Path(os.environ['OLD_BASELINE_ROOT']).resolve();sys.path.insert(0,str(Path(__file__).resolve().parent))
from render_authored_heart_contours import render_envelope
from prepare_old_reference_workspace import binary_nifti
s=json.loads((root/'phase00_authored_spec_v02.local.json').read_text(encoding='utf-8'));ct=nib.load(s['ct_path']);a=np.asarray(ct.dataobj)
...
out=root/'phase00_candidate_v02';out.mkdir(exist_ok=False)
...
for shift in [-3,0,3]:
 for hu in ([170,190,210] if shift==0 else [190]):
  bounds={'LV':v<=84+shift,'LA':v>84+shift,'RV':(~env['RA'])|(v+.2*u<=76+shift),'RA':(~env['RV'])|(v+.2*u>76+shift)}
  masks={}
  for ch in env:
...
(out/'provenance.json').write_text(json.dumps({'source_ct_sha256':s['ct_sha256'],'source_spec_sha256':hashlib.sha256((root/'phase00_authored_spec_v02.local.json').read_bytes()).hexdigest(),'builder_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'accepted':False,'status':'four_chamber_extent_candidates_pending_visual_review','automatic_comparator_used':False,'policy':s['policy'],'records':records},ensure_ascii=False,indent=2),encoding='utf-8')
