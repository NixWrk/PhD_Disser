...
from lateral_fat_prepare import PIPE,BASE,OUT,PREVIOUS,sha,read,write
...
def analyze(out=OUT):
    ...
    a=d[(d.kind=='literature')&(d.quadrature_points==256)]
    b=d[(d.kind=='literature')&(d.quadrature_points==1024)]
    check=b.merge(a,on=['frequency_hz','state','model','L_mm'],suffixes=('_1024','_256'),validate='one_to_one')
    check['difference_ohm']=check.Z_1024-check.Z_256
    check['difference_pct']=100*check.difference_ohm/check.Z_1024
    check['within_numeric_tolerance']=check.difference_ohm.abs()<=.02+.001*check.Z_1024.abs()
    ...
    qc=dict(complete=True,new_CEM_solutions=len(d),main_five_tissue_solutions=36,no_parameter_fitting=True,no_localization=True,
        ...
        numeric_tolerance='abs difference <= 0.02 ohm + 0.001 abs(Z_1024); numerical scenario, not instrument noise',
        global_spatial_convergence=False,physical_validation=False,
        inputs_sha256={n:sha(out/n) for n in ['plan.json','responses.csv','requests.csv','execution.json','material_inventory.csv','literature_values.csv','matrix_qc_q256.json','matrix_qc_q1024.json']},
