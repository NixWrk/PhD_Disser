...
"""
    for key in [which] if which else NAMES:
        ...
        roles={'20.10':'Геометрия C01, происхождение выбора и текущий прямой контроль.', '20.13':'Геометрические правила и новые маски C01.','20.14':'Две проверки: известные синтетические сопротивления и реальные измерения.',
               '20.16':'Проверка геометрических допущений по точности восстановления базовых сопротивлений: КТ-референс, исходная двуслойная формула, сокращения анатомии и быстрые операторы. Объяснено назначение всех 19 реализаций, приведены общие результаты на 65 состояниях и ограничения выбора методики.',
