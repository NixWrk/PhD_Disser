...
p=inputParser;addParameter(p,'OutputDir','output/exploratory/lateral_fat_inverse_20260916');
...
for ci=1:size(cases,1)
 ...
 limits=[1.3774104683195594,10;getrho("Lung (Deflated)"),24.074073806584366;getrho("Fat")*.75,getrho("Fat")*1.25];
 ...
 % Literature and interior starts; neither is fitted to a single array.
 starts=[log(rho(active));(lb+ub)/2];solutions=zeros(2,npar);values=zeros(2,1);flags=zeros(2,1);iters=zeros(2,1);
 ...
 [~,best]=min(values);x=solutions(best,:);[rr,J]=residual(x);rho(active)=exp(x);
 assert(all(flags>0),'At least one start failed to converge');
 S.Kparts=Coarse.Kparts;zc=vector(rho);S.Kparts=fineparts;qd=lastz-zc;
 sv=svd(J);boundary=abs(x-lb)<1e-5|abs(x-ub)<1e-5;
...
report=struct('complete',true,'identity',identity,'state',state,'fits',numel(rows),'global_optimum_certified',false,...
 'no_new_localization',true,'same_inspiratory_geometry',true,'rho3_scenario_is_not_population_interval',true,...
...
function [r,J]=residual(x)
 ...
 r=lastz-target;ix=[1,2];if numel(active)==3,ix=[1,2,3];end;J=lastJ(:,ix);
end
