function audit_nonlinear()
here=fileparts(mfilename('fullpath'));root=fileparts(fileparts(here));cd(root);addpath('src','vendor_stl_eidors');
cfg=trkg4_config('nik');trkg4_prepare_runtime(cfg);tag='disjoint_restartable_manifest90';
fit=readtable(fullfile('output',['nik_trkg4_inverse_inhale_fit_summary_' tag '.csv']));
expected=readtable(fullfile('output',['nik_trkg4_inverse_inhale_comparison_' tag '.csv']));
shared=load(fullfile('output','nik_trkg4_right_rib_050mm_full_v5_1mm_local2mm.mat'),'fmdl_mesh_units','elem_sigma','tissue_id','tissue_names');
fmdl=shared.fmdl_mesh_units;pose=local_build_pose_context(fmdl,local_load_reference_grid(root),cfg.subject.stl.lungs);
x=[fit.rho_soft_ohm_m fit.rho_lungs_ohm_m fit.centre_u_mm fit.centre_v_mm fit.phi_deg];
state=local_pose_state(pose,x(3),x(4),x(5));sizes=expected.L_mm;els=cell(numel(sizes),1);
for k=1:numel(sizes)
 centres=local_grid_centres(pose,state,sizes(k));
 els{k}=electrode_faces_by_area(fmdl,centres,cfg.electrode_area,cfg.z_contact,pose.patch_context);
end
model=trkg4_scale_fmdl_to_si(fmdl,cfg);[model.stimulation,model.meas_select]=trkg4_make_4electrode_stimulation(cfg);
soft=find(strcmp(shared.tissue_names,'soft_tissue'),1);lung=find(strcmp(shared.tissue_names,'lungs'),1);
rhos=[x(2),17.4067,14,12,10.29373007857537,10];zs=zeros(numel(sizes),numel(rhos));
for j=1:numel(rhos)
 sigma=shared.elem_sigma;sigma(shared.tissue_id==soft)=1/x(1);sigma(shared.tissue_id==lung)=1/rhos(j);
 for k=1:numel(sizes)
  model.electrode=els{k};v=fwd_solve(mk_image(model,sigma));zs(k,j)=real(v.meas(1));eidors_cache('clear_all');
 end
 fprintf('AUDIT NONLINEAR rho_lung=%.8g Z=%s\n',rhos(j),mat2str(zs(:,j)',8));
end
result=struct('purpose','Finite-change counterexample; fixed inhale geometry, rho_soft and contacts; no physiological inference', ...
 'L_mm',sizes,'rho_lung_values',rhos,'Z_ohm',zs,'canonical_max_abs_error_ohm',max(abs(zs(:,1)-expected.Z_FEM_ohm)));
fid=fopen(fullfile(here,'nonlinear_checks.json'),'w');fwrite(fid,jsonencode(result),'char');fclose(fid);
fprintf('AUDIT NONLINEAR COMPLETE\n');
end
