...
- `18_wall_spatial_map_w2.ipynb` показывает скан «плотность × разрешение», развёртку
  карты `h` по поверхности тела в цилиндрической системе координат и карту шума по тем
  же бинам; фазы в нём не сравниваются и карта `Δ` не строится;
- outputs этих семнадцати notebooks коммитятся как удобное представление результатов, но
  источником чисел остаются воспроизводимые артефакты в `results/`;
- после обновления batch-артефактов все семнадцать notebooks нужно выполнить заново и
  проверить,
