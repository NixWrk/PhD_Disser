...
TARGET = Path(os.environ.get(
    "TRKG4_REPORT_TARGET",
    ROOT.parent / "Colab Notebooks" / "20.10_КТ_FEM_локализация_боковых_матриц_МГТУ.ipynb"))
...
**Исходный расчёт ноутбука (отдельное двухтканное сравнение — в 11.3.1):** расчёт, выбранный через `TRKG4_RESULT_TAG`; по умолчанию
...
# materials; the strictly two-material controlled comparison follows in 11.3.1.
...
md(r"""
### 11.3.1. Проверка на двух тканях: переменная глубина и конечное лёгкое

**Допущение автора для этого сравнения:** существуют только два значения удельного
сопротивления. Лёгочная маска получает $\rho_2$, всё остальное внутри тела — $\rho_1$,
включая кости, сердце и ткани ниже диафрагмы. Следовательно, «три слоя» здесь означают
**мягкие ткани → лёгкое → мягкие ткани**, причём $\rho_3=\rho_1$.
Это отдельная двухтканная постановка; четырёхтканный FEM выше сохранён как исходный расчёт.

**Входы и статус:** используются та же КТ на вдохе, те же девять размеров, общий центр,
ориентация и CEM-площадки примера 11.3. Два сопротивления перенесены из исходной
четырёхтканной подгонки и фиксированы. Новая подгонка по эксперименту не выполнялась.
Все представленные ниже числа — результаты математических моделей, не независимая
валидация анатомии, параметров или измерений.

В текущем сечении при $s=-70\ldots70$ мм первая глубина до лёгкого меняется примерно
от 54 до 20 мм. Первый лёгочный интервал меняется примерно от 112 до 11 мм на концах
сборки; в центре он составляет около 116 мм. Луч пересекает лёгочную маску четыре или
шесть раз: одного конечного лёгочного интервала недостаточно, чтобы описать весь луч.
Наличие пересечений установлено по STL; не все выходы из маски можно назвать диафрагмой.

**Что именно рассчитывается.** Плоская модель с постоянными толщинами решается через
преобразование Ханкеля. Если $d_j$ — толщина слоя, $\lambda$ — пространственная частота,
то спектральная величина $T_j$, имеющая единицы удельного сопротивления, получается снизу вверх:

$$
T_N=\rho_N,\qquad
T_j(\lambda)=\rho_j\,
\frac{T_{j+1}(\lambda)+\rho_j\tanh(\lambda d_j)}
     {\rho_j+T_{j+1}(\lambda)\tanh(\lambda d_j)}.
$$

Для трёх слоёв $d_1=h$, $d_2=t_{lung}$, $\rho_3=\rho_1$. Поверхностное ядро точечного
источника, после выделения однородной особенности, равно

$$
G(r)=\frac1{2\pi}\left[\frac{\rho_1}{r}
+\int_0^\infty [T_1(\lambda)-\rho_1]J_0(\lambda r)\,d\lambda\right].
$$

Четырёхэлектродный импеданс собирается как
$Z=G_{AM}-G_{BM}-G_{AN}+G_{BN}$. Длины внутри интеграла заданы в метрах.
При $t_{lung}=0$ получается однородное полупространство, при
$t_{lung}\to\infty$ — обычная двухслойная формула. Эти пределы проверены тестами.
Такая рекурсия для горизонтальных слоёв используется и в
[первичной реализации SimPEG](https://github.com/simpeg/simpeg/blob/main/simpeg/electromagnetics/static/resistivity/simulation_1d.py).

Сопоставлены следующие варианты:

- постоянное $h$: ближайшее расстояние до поверхности и отдельно глубина центрального луча;
- исходный 11.3: среднее $h$ отдельно для каждой пары электродов;
- усреднение самого ядра $G(r,h(s))$ вместо предварительного усреднения $h$;
- конечный первый лёгочный интервал: постоянный в центре или усреднённый по паре;
- все чередующиеся интервалы вдоль луча: центральный плоский стек и усреднение
  локальных плоских ядер по реальному сечению;
- полный двухтканный FEM с замкнутой лёгочной маской;
- FEM на той же сетке с теми же контактами, но с продолжением лёгкого после первого входа
  вдоль направления `inward` сечения, до границы тела.

**Физическая граница приближений:** плоские стеки решают соответствующую плоскую задачу.
Подстановка средних толщин и усреднение ядер — эвристики. Они не обеспечивают условий
непрерывности потенциала и нормального тока на исходных кривых границах. Простое выпрямление
кожи также не является эквивалентной заменой геометрии: при точной замене координат
$x=F(\xi)$, $J=\partial x/\partial\xi$, в уравнении появляется
$\widetilde\sigma=|\det J|J^{-1}\sigma J^{-T}$.
В FEM исходная трёхмерная форма учитывается непосредственно; маска назначается по центрам
тетраэдров. Сходимость по сгущению этой сетки пока не проверена.

Вторая FEM-геометрия — контролируемое изменение **всех дальних выходов** вдоль одного
направления, а не изолированное перемещение купола диафрагмы. Только разность двух FEM
сравнивает эту замену при одинаковых конечных контактах, теле и численном методе.
Аналитические формулы используют номинальные точечные электроды на плоскости; их
невязку с CEM нельзя целиком приписывать кривизне лёгкого.
""")
...
# Scientific prose is maintained as a complete reader-facing manuscript.
# Replacement indices refer to the computational notebook assembled above;
# require exact Markdown coverage so structural changes cannot silently misalign it.
import re
scientific_narrative_path = ROOT / 'notebooks' / 'TRKG4_inverse_inhale_scientific.md'
scientific_text = scientific_narrative_path.read_text(encoding='utf-8')
scientific_parts = re.split(r'^<!-- (replace|append): (\d+) -->\s*$', scientific_text, flags=re.M)
scientific_replacements = {}
scientific_appendices = []
for part_index in range(1, len(scientific_parts), 3):
    operation, target_index, body = scientific_parts[part_index:part_index + 3]
    target_index = int(target_index)
    if operation == 'replace':
        if target_index in scientific_replacements:
            raise ValueError(f'Duplicate scientific text block: {target_index}')
        scientific_replacements[target_index] = body.strip()
    else:
        scientific_appendices.append(body.strip())
markdown_indices = {i for i, cell in enumerate(cells) if cell.cell_type == 'markdown'}
if set(scientific_replacements) != markdown_indices:
    raise ValueError('Scientific text must cover every original Markdown cell; review structural changes')
for target_index, body in scientific_replacements.items():
    cells[target_index].source = body
for body in scientific_appendices:
    cells.append(nbf.v4.new_markdown_cell(body))

# The saved-scan chapter consists of Markdown and embedded scientific figures.
# Its producer never executes notebook cells or resumes the FEM scan.
import json
chapter_dir = ROOT / 'output/exploratory/scan_localization_20260910/scientific_notebook'
chapter_source = ROOT / 'notebooks/TRKG4_saved_scan_scientific.md'
chapter_manifest_path = chapter_dir / 'chapter_manifest.json'
if not chapter_manifest_path.is_file():
    raise FileNotFoundError('Run notebooks/build_saved_scan_chapter.py before rebuilding the scientific report')
chapter_manifest = json.loads(chapter_manifest_path.read_text(encoding='utf-8'))
for path, expected in [
        (chapter_source, chapter_manifest['source_sha256']),
        (ROOT / 'notebooks/build_saved_scan_chapter.py', chapter_manifest['producer_sha256']),
        (chapter_dir / 'chapter_cells.json', chapter_manifest['chapter_cells_sha256'])]:
    if hashlib.sha256(path.read_bytes()).hexdigest() != expected:
        raise ValueError(f'Saved-scan chapter is stale: {path.name}; rebuild it first')
chapter_cells = [nbf.from_dict(cell) for cell in json.loads((chapter_dir / 'chapter_cells.json').read_text(encoding='utf-8'))]
if any(cell.cell_type != 'markdown' for cell in chapter_cells):
    raise ValueError('Saved-scan chapter must not add computational cells')
chapter_position = [i for i, cell in enumerate(cells)
                    if cell.cell_type == 'markdown' and cell.source.startswith('## 21. Итоговые положения')]
if len(chapter_position) != 1:
    raise ValueError('Expected a single final-conclusions section for chapter insertion')
cells[chapter_position[0]:chapter_position[0]] = chapter_cells

scientific_wording = [['Если нужна точность полной кривой границы, это уже задача\n  FEM, а не метода изображений.', 'Для сохранения полной геометрии требуется решение пространственной краевой задачи,\n  например методом конечных или граничных элементов; слоистый ряд решает свою идеализированную постановку.'], ['Это не похоже на\n  CT-геометрию и показывает взаимную компенсацию', 'Эта оценка существенно отличается от геометрического расстояния; совместная подгонка допускает компенсацию'], ['### Минимальная ошибка после отдельной калибровки каждой модели', '### Остатки после отдельной идентификации параметров'], ['### Технический вывод сравнения', '### Интерпретация параметрической подгонки'], ['На выдохе максимум сохраняется, следовательно, он не создаётся только выбором интервала вдоха.', 'Повышенное значение при 60 мм наблюдается в обоих принятых дыхательных состояниях; его причина по агрегированным уровням не установлена.'], ['Причина поведения решателя численная: при выборе конечного набора поверхностных граней\n    целевая функция по координатам электродов кусочно-постоянна и меняется скачками.', 'Выявлен численный механизм: при выборе конечного набора поверхностных граней\n    электрический отклик по позе имеет постоянные участки и скачки; геометрический априорный штраф может изменяться между ними.'], ['RMSE относительно расстояния кожа–лёгкое между контурами на показанном срезе:', 'где аппроксимируемая глубина и коэффициенты имеют единицы миллиметров, $s$ — продольная координата в миллиметрах, $t$ — безразмерная координата. RMSE аппроксимации расстояния кожа–лёгкое на показанном срезе:'], [',\\qquad t=s/70.$$', ',\\qquad t=s/70,$$']]
def scientific_markdown(text):
    # A flush-left heading followed by an indented f-string body otherwise
    # turns scientific prose into a Markdown code block.
    from textwrap import dedent
    first, separator, rest = text.partition('\n')
    return first + separator + dedent(rest)


def scientific_code_format(source):
    pattern = r"(Markdown\((?:fr|rf|f|r)?)(\"{3}|'{3})(.*?)(\2)"
    return re.sub(pattern, lambda match: match[1] + match[2]
                  + scientific_markdown(match[3]) + match[4], source, flags=re.S)


for cell in cells:
    if cell.cell_type == 'code':
        for old, new in scientific_wording:
            cell.source = cell.source.replace(old, new)
        cell.source = scientific_code_format(cell.source)

# Reader links are relative to the canonical notebook in Colab Notebooks.
for cell in cells:
