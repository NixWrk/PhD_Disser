...
- Исторический интерактивный notebook для `copd1` и три выполненных batch-notebook-отчёта,
  включая locked ConvexAdam benchmark и явный QC-verdict.
...
1. Реализовать новый заранее специфицированный registration-кандидат с отдельной обработкой
   лёгкого/грудной стенки, плеврального скольжения и диафрагмальных отделов. Expert 13-case
   test не использовать для подбора его параметров.
2. До реальных данных проверить направление поля, отсутствие folding и tangential sliding
   на synthetic phantom с известной деформацией.
