...
BOUNDARY_TOLERANCE = float(64*np.finfo(float).eps)
...
def _overlap(points, target, center, radii, rotation):
    ...
    candidate = np.sum((((points-center) @ rotation)/radii)**2, axis=1) <= 1+BOUNDARY_TOLERANCE
...
def _scanline_overlap(target_points, sampling, center, radii, rotation):

    """Count the FULL ellipsoid on the same infinite lattice by z intervals.



    For fixed integer x,y the ellipsoid inequality is a quadratic in z. Count

    all integer z between its roots. Target intersection still uses the original

    physical point test. This skips empty space without clipping candidates to

    the mask ROI or changing grid spacing. Floating boundary tolerance is shared

    with dense evaluation. The fitting domain was already bounded by _lattice.

    """

    frame = np.asarray(sampling["frame"])

    origin = np.asarray(sampling["origin_mm"])

    step = sampling["grid_step_mm"]

    c = (center-origin) @ frame / step

    mapping = (frame.T @ rotation)/radii*step

    matrix = mapping @ mapping.T

    extent = np.linalg.norm((frame.T @ rotation)*radii, axis=1)/step

    lower = np.ceil(c[:2]-extent[:2]-1).astype(np.int64)

    upper = np.floor(c[:2]+extent[:2]+1).astype(np.int64)

    ix = np.arange(lower[0], upper[0]+1)

    iy = np.arange(lower[1], upper[1]+1)

    dx = np.repeat(ix, len(iy))-c[0]

    dy = np.tile(iy, len(ix))-c[1]

    b = matrix[2, 0]*dx+matrix[2, 1]*dy

    constant = matrix[0, 0]*dx*dx+2*matrix[0, 1]*dx*dy+matrix[1, 1]*dy*dy

    disc = b*b-matrix[2, 2]*(constant-(1+BOUNDARY_TOLERANCE))

    valid = disc >= 0

    root = np.sqrt(disc[valid])

    zlo = np.ceil(c[2]+(-b[valid]-root)/matrix[2, 2]).astype(np.int64)

    zhi = np.floor(c[2]+(-b[valid]+root)/matrix[2, 2]).astype(np.int64)

    nc = int(np.maximum(zhi-zlo+1, 0).sum())

    ni = int(np.count_nonzero(np.sum((((target_points-center) @ rotation)/radii)**2,

                                     axis=1) <= 1+BOUNDARY_TOLERANCE))

    nt = len(target_points)

    if ni > min(nt, nc):

        raise FloatingPointError("Inconsistent scanline/intersection counts")
...
def fit_overlap_ellipsoid(mask, affine, initial, *, method="Л4а",
    ...
    target_points = p[target].copy()

    del p, target

    result["sampling"].update(candidate_counting="full_lattice_integer_scanline_intervals",

        intersection_point_count=len(target_points), boundary_tolerance=BOUNDARY_TOLERANCE)
    ...
    def objective(x):
        ...
        return 1-_scanline_overlap(target_points, result["sampling"], *space.decode(x))["IoU_sampled"]
    ...
    if best is not None:
        ...
        result.update(status="completed", parameters=parameters,
                      ...
                      fit_metrics=_scanline_overlap(target_points, result["sampling"], *space.decode(best[1])))
