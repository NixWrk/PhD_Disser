...
- Пространственная карта Δтолщины по телу: локальный шум `2.626 мм` одного порядка с
  ожидаемым эффектом при текущем разбиении. Это ошибка медианы в неоднородных бинах, а
  не предел измерителя (`D-045`); разрешение карты определяется протоколом W2, сама
  карта `Δ` остаётся закрытой до Gate 1B.
...
2. Трек грудной стенки ведётся параллельно J1.2, а не после него. Gate 1 в текущем виде
   ...
   свойство измерителя, поэтому перед (b) и (c) выполняется W2: выбор разрешения
   3D-карты в spine-anchored цилиндрической системе координат.
