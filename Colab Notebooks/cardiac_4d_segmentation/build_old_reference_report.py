...
def build(root, output_dir):
    ...
    review = json.loads((root/'body_review_v06/anatomical_review.json').read_text(encoding='utf-8'))
    ...
    folders = {'Левый желудочек':'sequence02_phase00_LV_body_v04',
               'Правый желудочек':'sequence02_phase00_RV_body_v04'}
    rows = []
    ...
    for section in protocol.split('\n## '):
        ...
        if section.startswith('Первые сохранённые контуры'):
            ...
            for figure, start in [(2,4),(3,8),(4,12)]:
                picture(f'body_review_v06/RV_before_after_{start}_{start+3}.jpg',
                    f'**Рисунок {figure}. Исправление контуров ПЖ на срезах {start}–{start+3}.** '
                    'Верхний ряд — исходная КТ. В нижнем ряду оранжевый пунктир показывает прежнюю '
                    'маску, голубая линия — исправленную; нижний порог в обоих случаях равен 190 HU. '+
                    ('Изменены положение области у верхушки и охват видимой полости. Неровные границы '
                     'на срезах 4–7 отражают действие порога и неоднозначность ткани у края полости. '
                     if start==4 else
                     'Пересмотрены участки, где прежняя опорная область ограничивала яркую часть полости. '
                     if start==8 else
                     'Исправлены соседние срезы, чтобы устранить искусственное сужение при переходе '
                     'от пересмотренной области к прежней разметке. ')+
                    'Контуры заданы агентом по изображению; независимая точность границ не установлена.')
            picture('body_review_v06/RV_orthogonal_review.jpg',
                '**Рисунок 5. Контроль ПЖ во фронтальной и сагиттальной плоскостях.** '
                'Голубым показана текущая маска при 190 HU. Оси подписаны индексами исходного массива, '
                'а пропорции изображения учитывают физические размеры вокселя. Видны ступенчатые '
                'границы и неоднозначные участки у нижнего края. Горизонтальная граница на уровне '
                '24-го среза ограничивает выполненную разметку и не соответствует установленному клапану. '
                'Этот контроль выявляет геометрические ограничения, но не заменяет экспертной проверки эндокарда.')
            picture('body_review_v06/oblique_partial_boundaries.jpg',
                '**Рисунок 6. Границы текущей разметки в двух косых продольных плоскостях.** '
            ...
            picture('thin_coarse_same_physical_planes.jpg',
                '**Рисунок 7. Толстосрезовая и тонкосрезовая реконструкции на одинаковых физических плоскостях.** '
            ...
            md('**Рисунок 8. Зависимость черновой разметки от нижнего порога HU.** '
    ...
    for page, extent in [(1,'4–11'),(2,'12–19'),(3,'20–27')]:
        picture(f'body_review_v06/axial_page{page}.jpg',
            f'**Рисунок {page+8}. Аксиальные плоскости {extent}.** '+
    ...
    md('## Практический итог\n\n'
       ...
       'содержательный шаг — независимо проверить пересмотренные границы и завершить разметку у клапанов и выходных трактов, '
       ...
       'первые редактируемые черновики — в `body_review_v06/slicer`, исходные точки и варианты масок — '
       'в `agent_drafts`. Файл `ОТКРЫТЬ_ЧЕРНОВИКИ.cmd` в `body_review_v06` открывает подготовленную сцену. '
    ...
    (root/'body_review_v06/report_numbers.json').write_text(json.dumps(rows,ensure_ascii=False,indent=2),encoding='utf-8')
