...
p=inputParser;addParameter(p,'OutputDir','output/exploratory/lateral_real_application_20260915');
addParameter(p,'PriorWorkerFile','');parse(p,varargin{:});
...
idfile=fullfile(out,'cem_identity.json');accepted_prior_identity=struct([]);
priorfile=char(p.Results.PriorWorkerFile);
if ~isempty(priorfile)
 if ~isfile(priorfile),priorfile=fullfile(root,priorfile);end
 assert(isfile(priorfile)&&isfile(idfile),'Explicit prior worker and run identity are required.');
 prior=jsondecode(fileread(idfile));
 assert(strcmp(prior.worker_sha256,trkg4_file_sha256(priorfile)),'Prior source does not match saved results.');
 cleanprior=rmfield(prior,'worker_sha256');cleannow=rmfield(identity,'worker_sha256');
 assert(isequal(cleanprior,jsondecode(jsonencode(cleannow))),'Scientific inputs changed; prior fits cannot be reused.');
 accepted_prior_identity=prior;
 local_json(fullfile(out,'cem_prior_identity.json'),prior);
 local_json(fullfile(out,'cem_resume_reason.json'),struct('reason','Rename finite-difference parameter loop index to avoid sharing the forward electrode loop variable. Numerical forward/objective formulas unchanged.',...
  'prior_source_sha256',prior.worker_sha256,'current_source_sha256',identity.worker_sha256,'explicit_prior_worker',true));
 local_json(idfile,identity);
elseif isfile(idfile)
 assert(isequal(jsondecode(fileread(idfile)),jsondecode(jsonencode(identity))),'Run identity changed; use a new output directory.');
 if isfile(fullfile(out,'cem_prior_identity.json')),accepted_prior_identity=jsondecode(fileread(fullfile(out,'cem_prior_identity.json')));end
...
for mi=[3 1]
 ...
 if model=="variable_transverse"
  ...
  for parameter_index=1:2,dx=zeros(2,1);dx(parameter_index)=1e-4;zp=forward(x+dx);zm=forward(x-dx);fd(:,parameter_index)=(zp-zm)/(2e-4);end
...
report=struct('complete',true,'status','exploratory_hypothesis_not_validated','identity',identity,...
 'accepted_prior_fit_identity',accepted_prior_identity,'baseline_reproduction',qc,'new_cached_parameter_evaluations',cache.Count,'global_optimum_proved',false,...
...
local_json(fullfile(out,'cem_execution.json'),report);
 ...
 function fit(starts,lo,hi)
  ...
  if isfile([stem '.json'])
   old=jsondecode(fileread([stem '.json']));
   assert(old.complete && (isequal(old.identity,jsondecode(jsonencode(identity))) || isequal(old.identity,accepted_prior_identity)));
   previous=readtable([stem '_predictions.csv'],'TextType','string');rx=log(old.best.rho(:));
   if scenario=="shared"
    [za,Ja]=forward(rx([1 2]));[zb,Jb]=forward(rx([1 3]));zz=[za;zb];JJ=[Ja;Jb];
   else,[zz,JJ]=forward(rx);end
   priorJ=[previous.dZ_dlogrho1 previous.dZ_dlogrho2];
   resumeZ=max(abs(zz-previous.predicted_ohm));resumeJ=norm(JJ-priorJ,'fro')/norm(priorJ,'fro');
   assert(resumeZ<1e-7 && resumeJ<1e-7,'Reused fit disagrees with the current direct response.');
   local_json([stem '_resume_qc.json'],struct('prediction_max_error_ohm',resumeZ,'jacobian_relative_error',resumeJ,'prior_fit_worker_sha256',old.identity.worker_sha256));
   return;
  end
  solutions=struct([]);
