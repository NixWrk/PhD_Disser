...
def display_revision_asset(kind: str, out: Path = ASSET_DIR) -> None:
    ...
    if kind == "search":
        ...
        rows = [
            ["u", f"{search['u_range_mm'][0]:g} … {search['u_range_mm'][1]:g} мм", "161 значений, шаг 1 мм"],
            ...
            ["позы", f"{search['status_shape'][0] * search['status_shape'][1]:,}".replace(",", " "), "полная решётка centre×φ"],
        ]
