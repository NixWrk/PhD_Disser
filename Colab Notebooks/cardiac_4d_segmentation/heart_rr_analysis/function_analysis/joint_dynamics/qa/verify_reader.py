...
c=Path('Colab Notebooks/cardiac_4d_segmentation').resolve();p=c/'heart_rr_analysis/function_analysis/joint_dynamics';qa=p/'qa';qa.mkdir(exist_ok=True)
...
assert sum('cardiac-function-analysis' in x.metadata.get('tags',[]) for x in nb.cells)==8
assert sum('georg-pilot-analysis' in x.metadata.get('tags',[]) for x in nb.cells)==2
baseline=json.loads(Path('Colab Notebooks/archive/private/21.03_before_joint_dynamics_20260910/baseline_hashes.json').read_text(encoding='utf-8'))
for f in [c/'heart_rr_analysis/chamber_volumes_by_rr.csv',c/'heart_rr_analysis/function_analysis/ventricular_function_estimates.csv']:
 assert hashlib.sha256(f.read_bytes()).hexdigest()==baseline[f.name],f
...
checks={'status':'passed','tests_passed':24,'notebook_cells':28,'joint_cells':3,'reader_plots':3,'notebook_plots_at_least':6,'boundary_pages':18,'local_links_checked':checked,'missing_links':missing,
        'legend_group_toggle':'passed','zoom_and_reset':'passed','source_code_in_html':False,'preserved_baseline_hashes':baseline,'browser_page_errors':errors,
