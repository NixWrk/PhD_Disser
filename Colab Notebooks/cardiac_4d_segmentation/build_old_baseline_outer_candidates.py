...
s=json.loads((root/'phase00_outer_spec_v02.local.json').read_text(encoding='utf-8'));ct=nib.load(root.parent/'sequence_02/phases/phase_00.nii.gz');a=np.asarray(ct.dataobj);spacing=np.linalg.norm(ct.affine[:3,:3],axis=0)
...
added_for_containment=int((blood&~outer).sum());outer|=blood
out=root/'phase00_outer_v02';out.mkdir(exist_ok=False)
...
fig.savefig(root/'phase00_outer_candidates_v02.jpg',dpi=120,pil_kwargs={'quality':89});plt.close(fig)
